--[[
  ...
  All icons use Roblox asset IDs.
  Smooth UI with rounded corners and subtle shadows.
...
-- Utility: Shadow (ImageLabel with slice scaling)
local function addShadow(parent)
	local shadow = Instance.new("ImageLabel")
	shadow.Name = "Shadow"
	shadow.BackgroundTransparency = 1
	shadow.Image = "rbxassetid://1316045217" -- subtle shadow asset
...
addUICorner(mainFrame, UDim.new(0, 20))
addShadow(mainFrame)
...
addUICorner(header, UDim.new(0, 20))

-- Moon icon (Roblox asset)
...
addUICorner(closeBtn, UDim.new(0, 10))
...
addUICorner(sidebar, UDim.new(0, 20))
addShadow(sidebar)

-- Sidebar icons asset IDs (replace with your own if needed)
local sidebarAssetIds = {
	6031094677, -- Home
	6031094680, -- Left arrow
	6031094683, -- Right arrow
	6031094686, -- Location
	6031094689, -- User
	6031094692, -- Star
	6031094695, -- Bell
...
for i, assetId in ipairs(sidebarAssetIds) do
	local yPos = (i-1)*48 + 8
	createSidebarIcon(assetId, yPos)
end
...
addUICorner(avatarSmall, UDim.new(0, 12))
...
addUICorner(userInfo, UDim.new(0, 16))
...
addUICorner(avatarLarge, UDim.new(0, 16))
...
userSubLabel.TextColor3 = colors.textGray
...
addUICorner(serverPanel, UDim.new(0, 16))
...
addUICorner(rightTopPanel, UDim.new(0, 16))
...
addUICorner(discordPanel, UDim.new(0, 16))
...
addUICorner(friendsPanel, UDim.new(0, 16))
